...
def handle_incoming_candle(data):
    ...
    try:
        ...
        ema10 = float(data.get('ema10') or 0.0)
        ...
        db = TradeDB()
        db.save_candle(symbol, timeframe, time_str, open_p, high_p, low_p, close_p, volume, delta, ema10=ema10, ema34=ema34, ema50=ema50)
    ...
    def do_GET(self):
        ...
        if url_path.startswith("/api/test/"):
            ...
            if test_type == "deepseek":
                ...
                try:
                    ...
                    ema10_m1, ema34_m1, ema50_m1 = get_all_emas(db, "M1")
                    ema10_m5, ema34_m5, ema50_m5 = get_all_emas(db, "M5")
            ...
            elif test_type in ["simulate-buy", "simulate-sell"]:
                try:
                    from trade_db import TradeDB
                    db = TradeDB()
                    lp = db.get_live_price("XAUUSD")
                    spot_price = float(lp["bid"]) if (lp and lp.get("bid")) else 4015.00
                    
                    direction = "BUY" if test_type == "simulate-buy" else "SELL"
                    offset = 5.0 if direction == "BUY" else -5.0
                    
                    test_trade = {
                        "type": direction,
                        "strategy": "Trend Continuation (Test Mode)",
                        "category": "scalp",
                        "entry_price": spot_price,
                        "stop_loss": spot_price - offset,
                        "take_profit_1": spot_price + offset,
                        "take_profit_2": spot_price + (2 * offset),
                        "take_profit_3": spot_price + (3 * offset),
                        "confidence": 100.0,
                        "is_test": True,
                        "reasons": ["🧪 Live System Pipeline Test (Admin Only)", "Volume Confirmed", "Delta Confirmed"],
                        "metrics_snapshot": {
                            "vwap": spot_price,
                            "cvd": 1500 if direction == "BUY" else -1500,
                            "score_reasons": "Test Mode Injected Trade",
                            "signal_classification": "Test Setup"
                        }
                    }
                    
                    saved_id = db.save_trade(test_trade)
                    
                    if hasattr(self.server, 'scanner') and self.server.scanner:
                        asyncio.run_coroutine_threadsafe(
                            self.server.scanner.broadcast_order_flow_signal(test_trade),
                            self.server.loop
                        )
                    ...
                    response_data = {
                        "status": "success",
                        "trade_id": saved_id,
                        "message": f"✅ Successfully generated {direction} test trade, saved to database, and broadcasted live to dashboard & subscribers!",
                        "trade_details": test_trade
                    }
                except Exception as e:
                    response_data = {"status": "error", "message": f"Failed to simulate trade: {str(e)}"}

            elif test_type == "pipeline-check":
                try:
                    from trade_db import TradeDB
                    from datetime import datetime
                    db = TradeDB()
                    c_m1 = len(db.get_candles("XAUUSD", "M1", limit=100))
                    c_m5 = len(db.get_candles("XAUUSD", "M5", limit=100))
                    active_t = len(db.get_active_trades())
                    closed_t = len(db.get_all_closed_trades())
                    rejected_t = len(db.get_rejected_signals(limit=1000))
                    lp = db.get_live_price("XAUUSD")
                    
                    response_data = {
                        "status": "success",
                        "pipeline_health": "100% OPERATIONAL",
                        "spot_price": lp["bid"] if (lp and lp.get("bid")) else "No live price yet",
                        "m1_candles_count": c_m1,
                        "m5_candles_count": c_m5,
                        "active_trades_count": active_t,
                        "closed_trades_count": closed_t,
                        "rejected_trades_count": rejected_t,
                        "scanner_status": "RUNNING" if (hasattr(self.server, 'scanner') and self.server.scanner) else "STOPPED",
                        "timestamp": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
                    }
                except Exception as e:
                    response_data = {"status": "error", "message": f"Pipeline check error: {str(e)}"}
        ...
        if url_path == "/api/rejected-signals":
            ...
            try:
                ...
                filter_val = query_components.get("filter", ["today"])[0]
                
                signals = db.get_rejected_signals(limit=50, time_filter=filter_val)
                self.wfile.write(json.dumps(signals).encode('utf-8'))
            except Exception as e:
                self.wfile.write(json.dumps({"status": "error", "message": str(e)}).encode('utf-8'))
            return

        # Recent Bot Executed Trades API
        if url_path == "/api/recent-trades":
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.end_headers()
            try:
...
<body class="notranslate">
    <div class="container">
        ...
        <div style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.12), rgba(69, 243, 255, 0.12)); border: 1px solid #66fcf1; border-radius: 12px; padding: 15px 20px; margin-bottom: 25px; text-align: center; box-shadow: 0 0 15px rgba(102, 252, 241, 0.15);">
            <div style="display: flex; gap: 15px; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                <div style="text-align: right;">
                    <h3 style="color: #66fcf1; font-size: 1.15rem; margin-bottom: 4px; font-weight: 700;">🔍 مركز تشخيص وسلامة الاتصال المباشر (Pipeline Diagnostic Panel)</h3>
                    <p style="color: #c5c6c7; font-size: 0.88rem; margin: 0;">فحص حالة معالجة الشموع، الاتصال بالمستودع والماسح الآلي دون أي إدخال تجريبي</p>
                </div>
                <div>
                    <button class="btn" style="background: #66fcf1; color: #0b0c10; width: auto; padding: 10px 24px; font-size: 0.95rem; font-weight: 700; box-shadow: 0 4px 12px rgba(102, 252, 241, 0.3); border-radius: 8px;" onclick="runTest('pipeline-check')">🔍 فحص سلامة الويب هوك والسيرفر</button>
                </div>
            </div>
        </div>
        ...
        <div class="grid">
            ...
            <div class="card" style="border-color: rgba(102, 252, 241, 0.3);">
                <h3>🔍 سلامة خط الويب هوك</h3>
                <p style="font-size:0.85rem; color:#8b9bb4; margin-bottom:15px;">فحص حالة الشموع المسجلة والـ Webhook والماسح الآلي</p>
                <button class="btn" onclick="runTest('pipeline-check')">فحص الويب هوك 🔍</button>
            </div>
        </div>
        ...
        <div style="margin-top: 35px; background: rgba(31, 40, 51, 0.4); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 12px; padding: 20px;">
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255, 255, 255, 0.05); padding-bottom: 12px; margin-bottom: 15px;">
                <h3 style="color: #66fcf1; font-size: 1.25rem; font-weight: 600;">📊 صفقات البوت المنفذة والمراقبة (Executed Bot Trades)</h3>
                <button class="btn" style="width: auto; padding: 6px 15px; font-size: 0.85rem;" onclick="loadRecentTrades()">🔄 تحديث الصفقات</button>
            </div>
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; text-align: right; font-size: 0.9rem;" id="recent-trades-table">
                    <thead>
                        <tr style="border-bottom: 2px solid rgba(255, 255, 255, 0.08); color: #8b9bb4;">
                            <th style="padding: 10px;">التوقيت</th>
                            <th style="padding: 10px;">الاستراتيجية</th>
                            <th style="padding: 10px;">الاتجاه</th>
                            <th style="padding: 10px;">الدخول</th>
                            <th style="padding: 10px;">الوقف (SL)</th>
                            <th style="padding: 10px;">الأهداف (TP1 / TP2 / TP3)</th>
                            <th style="padding: 10px; text-align: center;">الحالة</th>
                            <th style="padding: 10px;">الأسباب الفنية</th>
                        </tr>
                    </thead>
                    <tbody id="recent-trades-body">
                        <tr>
                            <td colspan="8" style="padding: 20px; text-align: center; color: #8b9bb4;">جاري تحميل صفقات البوت...</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        ...
        async function updateLivePrice() {
            try {
                ...
                if (data.status === 'success') {
                    ...
                    if (data.m1 && data.m5) {
                        waveSpan.innerText = "M1 (EMA10: $" + data.m1.ema10.toFixed(1) + " | EMA34: $" + data.m1.ema34.toFixed(1) + " | EMA50: $" + data.m1.ema50.toFixed(1) + ") | M5 (EMA10: $" + data.m5.ema10.toFixed(1) + " | EMA34: $" + data.m5.ema34.toFixed(1) + " | EMA50: $" + data.m5.ema50.toFixed(1) + ")";
                    } else if (data.ema34 && data.ema50) {
                        waveSpan.innerText = "EMA34: $" + data.ema34.toFixed(2) + " | EMA50: $" + data.ema50.toFixed(2);
                    } else {
                        waveSpan.innerText = "M1: -- | M5: --";
                    }
                ...
                document.getElementById("live-ema-wave").innerText = "M1: -- | M5: --";
        ...
        async function loadRejectedSignals() {
            ...
            const filterVal = document.getElementById("signal-filter").value;
            try {
                const response = await fetch('/api/rejected-signals?filter=' + filterVal);
                const data = await response.json();
                
                if (data.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="7" style="padding: 20px; text-align: center; color: #8b9bb4;">لا يوجد صفقات مرفوضة في قاعدة البيانات حالياً لهذه الفترة.</td></tr>`;
                    return;
                }
        ...
        async function loadRecentTrades() {
            ...
            try {
                ...
                if (data.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="8" style="padding: 20px; text-align: center; color: #8b9bb4;">لا يوجد صفقات مسجلة في قاعدة البيانات حالياً.</td></tr>`;
                    return;
                }
                
                let html = "";
                data.forEach(t => {
                    let timestampStr = t.timestamp;
                    const date = new Date(timestampStr.replace(' ', 'T') + 'Z').toLocaleString("ar-EG");
                    const signalColor = t.direction === 'BUY' ? '#10b981' : '#ef4444';
                    
                    let statusBadge = "";
                    if (t.status === 'active') {
                        statusBadge = `<span class="status-badge" style="background: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid #10b981;">نشطة (Active)</span>`;
                    } else if (t.status === 'pending') {
                        statusBadge = `<span class="status-badge" style="background: rgba(251, 191, 36, 0.2); color: #fbbf24; border: 1px solid #fbbf24;">معلقة (Pending)</span>`;
                    } else if (t.status.startsWith('tp')) {
                        statusBadge = `<span class="status-badge" style="background: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid #10b981;">${t.status.toUpperCase()} Hit</span>`;
                    } else if (t.status === 'closed') {
                        statusBadge = `<span class="status-badge" style="background: rgba(16, 185, 129, 0.4); color: #10b981;">مغلقة (Closed)</span>`;
                    } else if (t.status === 'sl_hit') {
                        statusBadge = `<span class="status-badge" style="background: rgba(239, 68, 68, 0.2); color: #ef4444; border: 1px solid #ef4444;">فاشلة ضرب وقف (SL Hit)</span>`;
                    } else {
                        statusBadge = `<span class="status-badge status-idle">${t.status}</span>`;
                    }
                    
                    const tp2Price = t.tp2_price ? `$${t.tp2_price.toFixed(2)}` : '--';
                    const tp3Price = t.tp3_price ? `$${t.tp3_price.toFixed(2)}` : '--';
                    const tps = `TP1: $${t.tp1_price.toFixed(2)}<br>TP2: ${tp2Price}<br>TP3: ${tp3Price}`;
                    const confluences = t.confluences ? t.confluences.replace(/ \\| /g, '<br>') : 'شروط فنية مستوفاة';
                    ...
                    html += `<tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.05); transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='none'">
                        ...
                        <td style="padding: 12px; text-align: right; color: #ef4444;">$${t.sl_price.toFixed(2)}</td>
                        <td style="padding: 12px; text-align: right; font-size: 0.85rem; line-height: 1.4;">${tps}</td>
                        <td style="padding: 12px; text-align: center;">${statusBadge}</td>
                        <td style="padding: 12px; color: #8b9bb4; font-size: 0.85rem; text-align: right; line-height: 1.4;">${confluences}</td>
                    </tr>`;
                });
                tbody.innerHTML = html;
            } catch (err) {
                tbody.innerHTML = `<tr><td colspan="8" style="padding: 20px; text-align: center; color: #ef4444;">فشل جلب الصفقات: ${err.message}</td></tr>`;
            }
        }
        ...
        // Polling recent trades status every 10 seconds
        loadRecentTrades();
    ...
    def do_POST(self):
        ...
        if url_path in ["/api/market-data", "/api/live-prices", "/api/ticks", "/api/live-footprint"]:
            ...
            valid_secrets = [expected_secret, "NEROFLOW_SECRET", "MySecretKey123", "NEROFLOW", "SECRET", "MySecretKey"]
            ...
            if url_path == "/api/live-footprint":
                ...
                if isinstance(data, list):
                    ...
                    if len(data) > 1:
                        ...
                        candle_delta_val = float(closed_c.get("delta") or 0.0)
                        final_volume = int(closed_c.get("volume") or 0)
                        ema10_val = float(closed_c.get("ema10") or 0.0)
                        ema34_val = float(closed_c.get("ema34") or 0.0)
                        ema50_val = float(closed_c.get("ema50") or 0.0)

                        # Save closed candle to DB directly
                        from trade_db import TradeDB
                        db = TradeDB()
                        db.save_candle(
                            ...
                            volume=final_volume, delta=candle_delta_val, footprint_data="{}",
                            ema10=ema10_val, ema34=ema34_val, ema50=ema50_val
                        )
                        logger.info(f"Closed {tf} candle [{time_str}] processed & saved directly. Vol: {final_volume}, Delta: {candle_delta_val}, EMA10: {ema10_val}")
