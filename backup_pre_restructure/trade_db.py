...
class TradeDB:
    ...
    def _create_table(self) -> None:
        ...
        try:
            ...
            # Migration to add ema34 and ema50 to candles
            try:
    ...
    def log_trade(
        ...
        INSERT INTO trades (timestamp, direction, entry_price, sl_price, tp1_price, tp2_price,
                            tp3_price, status, trade_type, asset)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)
        """
        try:
            cursor = self.conn.execute(
                insert_sql, (timestamp, direction, entry, sl, tp1, tp2, tp3, trade_type, asset)
            )
            self.conn.commit()
    ...
    def save_candle(self, symbol: str, timeframe: str, time_str: str, open_p: float, high_p: float, low_p: float, close_p: float, volume: int, delta: float = 0.0, footprint_data: str = None, ema34: float = 0.0, ema50: float = 0.0) -> bool:
        """Insert or update a candle in the DB, preserving existing footprint_data or delta/volume if needed."""
        try:
            # Check if candle already exists
            select_query = "SELECT footprint_data, delta, volume, ema34, ema50 FROM candles WHERE symbol = ? AND timeframe = ? AND time = ?"
            ...
            query = """
            INSERT OR REPLACE INTO candles (symbol, timeframe, time, open, high, low, close, volume, delta, footprint_data, ema34, ema50)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
            self.conn.execute(query, (symbol, timeframe, time_str, open_p, high_p, low_p, close_p, volume, delta, footprint_data, ema34, ema50))
            self.conn.commit()
    ...
    def save_live_price(self, symbol: str, bid: float, ask: float, spread: float, server_time: str, ema34: float = 0.0, ema50: float = 0.0) -> bool:
        """Save/update the latest live price for a symbol."""
        query = """
        INSERT OR REPLACE INTO live_prices (symbol, bid, ask, spread, server_time, ema34, ema50, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """
        updated_at = datetime.utcnow().isoformat()
        try:
            self.conn.execute(query, (symbol, bid, ask, spread, server_time, ema34, ema50, updated_at))
            self.conn.commit()
            return True
        except sqlite3.Error as e:
    ...
    def get_rejected_signals(self, limit: int = 50) -> List[Dict[str, Any]]:
        """Retrieve recent rejected signals from the database."""
