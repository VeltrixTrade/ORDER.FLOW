...
class MarketScanner:
    ...
    async def start(self, scan_interval_seconds: int = 300):
        ...
        while self.is_running:
            try:
                # High-Frequency Active Trade Monitor (Runs every 60 seconds)
    ...
    async def scan_and_signal(self, trade_type: str, force: bool = False) -> tuple[bool, Any]:
        ...
        try:
            ...
            if force:
                ...
                # Fetch recent candles to get last M5 candle
...
"""
    ...
    async def broadcast_order_flow_signal(self, signal: Dict[str, Any]):
        ...
        try:
            ...
            # 2b. Enforce limit of maximum 2 active rules-based trades (exclude AI chatbot trades)
            rules_trades_count = sum(1 for t in active_trades if t.get('source') != 'ai' and t.get('status') in ['active', 'pending'])
            if rules_trades_count >= 2:
                logger.info(f"Skipping signal - Already at maximum of 2 rules-based active/pending trades ({rules_trades_count})")
                return
            ...
            final_msg = f"""
⚡ <b>تنبيه صفقة تدفق السيولة | Order Flow Signal</b>
{'═'*35}

🤖 <b>الاستراتيجية | Strategy:</b> Order Flow Imbalance
...
📝 <b>المؤشرات الفنية | Order Flow Metrics:</b>
• VWAP: ${signal['metrics_snapshot']['vwap']:.2f}
• CVD: {signal['metrics_snapshot']['cvd']:+d}
• VAL: ${signal['metrics_snapshot']['val']:.2f} | VAH: ${signal['metrics_snapshot']['vah']:.2f}

{'─'*35}
⚠️ <i>تنويه: تحليل تلقائي لتدفق السيولة. يرجى استخدام إدارة مخاطر صارمة.</i>
"""
            # 4. Broadcast to subscribers
            logger.info(f"📢 Broadcasting Order Flow Signal: {signal['type']} @ {signal['entry_price']}")
